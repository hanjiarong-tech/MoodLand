! function(root, factory) {
	"object" == typeof exports && "object" == typeof module ? module.exports = factory() : "function" == typeof define && define.amd ? define([], factory) : "object" == typeof exports ? exports.VueCalendarHeatmap = factory() : root.VueCalendarHeatmap = factory()
}(this, function() {
	return function(modules) {
		...
		var DEFAULT_RANGE_COLOR = ["#ebedf0", "#c0ddf9", "#73b3f3", "#3886e1", "#17459e"],
			...
			DAYS_IN_ONE_YEAR = 100,
			DAYS_IN_WEEK = 7,
			SQUARE_SIZE = 8
	}, function(module, exports, __webpack_require__) {
		__webpack_require__(8), __webpack_require__(7);
		var Component = __webpack_require__(9)(__webpack_require__(6), __webpack_require__(10), "data-v-a9cfea66", null);
		...
		var __WEBPACK_IMPORTED_MODULE_0__consts__ = __webpack_require__(1),
		...
		Object.defineProperty(__webpack_exports__, "__esModule", {
			...
			function(global) {
				...
				var __WEBPACK_IMPORTED_MODULE_0__components_CalendarHeatmap_vue__ = __webpack_require__(2),
		...
		(function(global) {
			function microtaskDebounce(fn) {
				var called = !1;
				return function() {
					called || (called = !0, window.Promise.resolve()
						.then(function() {
							called = !1, fn()
						}))
				}
			...
			function getOffsetParent(element) {
				...
				for (var noOffsetParent = isIE(10) ? document.body : null, offsetParent = element.offsetParent; offsetParent === noOffsetParent && element.nextElementSibling;) offsetParent = (element = element.nextElementSibling)
					.offsetParent;
			...
			function findCommonOffsetParent(element1, element2) {
				...
				return element1root.host ? findCommonOffsetParent(element1root.host, element2) : findCommonOffsetParent(element1, getRoot(element2)
					.host)
			}
			...
			function runModifiers(modifiers, data, ends) {
				return (void 0 === ends ? modifiers : modifiers.slice(0, findIndex(modifiers, "name", ends)))
			...
			function getSupportedPropertyName(property) {
				for (var prefixes = [!1, "ms", "Webkit", "Moz", "O"], upperProp = property.charAt(0)
					.toUpperCase() + property.slice(1), i = 0; i < prefixes.length; i++) {
			...
			function setupEventListeners(reference, options, state, updateBound) {
				state.updateBound = updateBound, getWindow(reference)
					.addEventListener("resize", state.updateBound, {
						passive: !0
					});
			...
			function removeEventListeners(reference, state) {
				return getWindow(reference)
					.removeEventListener("resize", state.updateBound), state.scrollParents.forEach(function(target) {
			...
			function applyStyle(data) {
				return setStyles(data.instance.popper, data.styles), setAttributes(data.instance.popper, data.attributes), data.arrowElement && Object.keys(data.arrowStyles)
					.length && setStyles(data.arrowElement, data.arrowStyles), data
			}
			...
			function clockwise(placement) {
				var counter = arguments.length > 1 && void 0 !== arguments[1] && arguments[1],
					...
					arr = validPlacements.slice(index + 1)
			...
			function parseOffset(offset, popperOffsets, referenceOffsets, basePlacement) {
				...
				var splitRegex = /\s*,\s*|\s+/,
					ops = -1 !== divider ? [fragments.slice(0, divider)
						.concat([fragments[divider].split(splitRegex)[0]]), [fragments[divider].split(splitRegex)[1]].concat(fragments.slice(divider + 1))
					] : [fragments];
			...
			function removeClasses(el, classes) {
				var newClasses = convertToArray(classes),
					classList = void 0;
				classList = convertToArray(el.className instanceof SVGAnimatedString ? el.className.baseVal : el.className), newClasses.forEach(function(newClass) {
					var index = classList.indexOf(newClass); - 1 !== index && classList.splice(index, 1)
				}), el instanceof SVGElement ? el.setAttribute("class", classList.join(" ")) : el.className = classList.join(" ")
			}
			...
			function handleGlobalClose(event) {
				...
				requestAnimationFrame(function() {
					for (var popover = void 0, i = 0; i < openPopovers.length; i++)
						if (popover = openPopovers[i], popover.$refs.popover) {
				...
				Popper = function() {
					function Popper(reference, popper) {
						...
						classCallCheck(this, Popper), this.scheduleUpdate = function() {
								...
								isCreated: !1,
								scrollParents: []
							}, this.reference = reference && reference.jquery ? reference[0] : reference, this.popper = popper && popper.jquery ? popper[0] : popper, this.options.modifiers = {}, Object.keys(_extends({}, Popper.Defaults.modifiers, options.modifiers))
			...
			Popper.Utils = ("undefined" != typeof window ? window : global)
				.PopperUtils, Popper.placements = placements, Popper.Defaults = Defaults;
			...
			var _typeof = "function" == typeof Symbol && "symbol" === _typeof2(Symbol.iterator) ? function(obj) {
				...
				Tooltip = function() {
					...
					return createClass$1(Tooltip, [{
						...
						key: "_init",
						value: function() {
							var events = "string" == typeof this.options.trigger ? this.options.trigger.split(" ")
								.filter(function(trigger) {
									return -1 !== ["click","focus"].indexOf(trigger)
								}) : [];
						...
						value: function(reference, template) {
							...
							return tooltipNode.id = "tooltip_" + Math.random()
								.toString(36)
								.substr(2, 10), tooltipNode.setAttribute("aria-hidden", "true"), this.options.autoHide && -1 !== this.options.trigger.indexOf("click"), tooltipNode
						}
					}, {
						key: "_setContent",
						value: function(content, options) {
							var _this = this;
							this.asyncContent = !1, this._applyContent(content, options)
								.then(function() {
								...
								if (1 === title.nodeType) {
									...
									if ("function" == typeof title) {
										...
										return void(result && "function" == typeof result.then ? (_this2.asyncContent = !0, options.loadingClass && addClasses(rootNode, options.loadingClass), options.loadingContent && _this2._applyContent(options.loadingContent, options), result.then(function(asyncResult) {
												...
												.then(resolve)
												.catch(reject)) : _this2._applyContent(result, options)
											.then(resolve)
											.catch(reject))
									}
						...
						key: "_noLongerOpen",
						value: function() {
							var index = openTooltips.indexOf(this); - 1 !== index && openTooltips.splice(index, 1)
						}
					}, {
						key: "_hide",
						value: function() {
							...
							return null !== disposeTime && (this._disposeTimer = setTimeout(function() {
								_this4._tooltipNode && (_this4._tooltipNode.removeEventListener("click", _this4.hide), _this4._tooltipNode.removeEventListener("click", _this4.hide), _this4._tooltipNode.parentNode.removeChild(_this4._tooltipNode), _this4._tooltipNode = null)
						...
						value: function() {
							...
							}), this._events = [], this._tooltipNode ? (this._hide(), this._tooltipNode.removeEventListener("click", this.hide), this._tooltipNode.removeEventListener("click", this.hide), this.popperInstance.destroy(), this.popperInstance.options.removeOnDestroy || (this._tooltipNode.parentNode.removeChild(this._tooltipNode), this._tooltipNode = null)) : this._noLongerOpen(), this
						}
					}, {
						key: "_findContainer",
							...
							events.forEach(function(event) {
								switch (event) {
									case "focus":
										...
										break;
									case "click":
										directEvents.push("click"), oppositeEvents.push("click"), _this6.options.hideOnTargetClick && oppositeEvents.push("click");
								}
			...
			var state = {
				...
				GlobalVue$1 = null;
			"undefined" != typeof window ? GlobalVue$1 = window.Vue : void 0 !== global && (GlobalVue$1 = global.Vue), GlobalVue$1 && GlobalVue$1.use(plugin$2);
			var isIOS = !1;
				...
				methods: {
					...
					$_show: function() {
						...
						if (clearTimeout(this.$_disposeTimer), !this.isOpen) {
							...
							if (openGroup)
								for (var popover = void 0, i = 0; i < openPopovers.length; i++) popover = openPopovers[i], popover.openGroup !== openGroup && (popover.hide(), popover.$emit("close-group"));
					...
					$_hide: function() {
						var _this4 = this;
						if (this.isOpen) {
							var index = openPopovers.indexOf(this); - 1 !== index && openPopovers.splice(index, 1), this.isOpen = !1, this.popperInstance && this.popperInstance.disableEventListeners(), clearTimeout(this.$_disposeTimer);
					...
					$_addEventListeners: function() {
						var _this5 = this,
							...
							directEvents = [],
							oppositeEvents = [];
						("string" == typeof this.trigger ? this.trigger.split(" ")
							.filter(function(trigger) {
								return -1 !== ["click","focus"].indexOf(trigger)
							}) : [])
			...
			var commonjsGlobal = "undefined" != typeof window ? window : void 0 !== global ? global : "undefined" != typeof self ? self : {},
				lodash_merge = function(fn, module) {
					...
					function mapCacheDelete(key) {
						var result = getMapData(this, key)
							.delete(key);
						return this.size -= result ? 1 : 0, result
					}
					...
					function baseIsNative(value) {
						return !(!isObject(value) || isMasked(value)) && (isFunction(value) ? reIsNative : reIsHostCtor)
							.test(toSource(value))
					}
					...
					function cloneArrayBuffer(arrayBuffer) {
						...
						return new Uint8Array(result)
							.set(new Uint8Array(arrayBuffer)), result
					}
					...
					var freeGlobal = "object" == (void 0 === commonjsGlobal ? "undefined" : _typeof2(commonjsGlobal)) && commonjsGlobal && commonjsGlobal.Object === Object && commonjsGlobal,
						...
						reIsNative = RegExp("^" + funcToString.call(hasOwnProperty)
		...
		var __WEBPACK_IMPORTED_MODULE_0_v_tooltip__ = __webpack_require__(5),
			__WEBPACK_IMPORTED_MODULE_1__Heatmap__ = __webpack_require__(3),
			__WEBPACK_IMPORTED_MODULE_2__consts_js__ = __webpack_require__(1);
		__WEBPACK_IMPORTED_MODULE_0_v_tooltip__.a.enabled = window.innerWidth > 768, __webpack_exports__.default = {
			...
			computed: {
				...
				monthsLabelWrapperTransform: function() {
					return {
						horizontal: "translate(" + this.LEFT_SECTION_WIDTH/2 + ", 0)",
				...
				yearWrapperTransform: function() {
					return "translate(" + this.LEFT_SECTION_WIDTH/2 + ", " + this.TOP_SECTION_HEIGTH + ")"
				},
			...
			methods: {
				tooltipOptions: function(day) {
					return !!this.tooltip && {
						...
						content: "<b>" + this.getMood(day.count) + " " + this.tooltipUnit + "</b> " + this.lo.on + " " + this.lo.months[day.date.getMonth()] + " " + day.date.getDate() + ", " + day.date.getFullYear(),
						delay: {
							show: 150,
							hide: 50
						}
				...
				getMood(count){
					var moodtype = ["Surprise", "Fear", "Disgusted", "Happy", "Sad", "Angry", "Neutral"];
					var res = 'No record';
					if(count>0){
					  res = moodtype[Math.floor((count-1)/4)]
					}
					return res;
				...
				getMonthLabelPostion: function(month) {
					...
					return position.x = this.vertical ? 3 : this.SQUARE_SIZE * (month.index-1), position.y = this.vertical ? this.SQUARE_SIZE * this.heatmap.weekCount - this.SQUARE_SIZE * month.index - this.SQUARE_SIZE / 4 : this.SQUARE_SIZE - this.SQUARE_BORDER_SIZE, position
				}
			}
		}
			...
			render: function() {
				...
				return _c("svg", {
					...
					return _c("text", {
						...
						attrs: {
							x: _vm.getMonthLabelPostion(month)
								.x,
							y: _vm.getMonthLabelPostion(month)
								.y
						}
